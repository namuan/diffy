...
class GuiIntegrationTest(unittest.TestCase):
    ...
    def test_open_pull_request_in_gui(self):
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(window.expand_all_button.text(), "Expand all")
            self.assertEqual(window.collapse_all_button.text(), "Collapse all")
            window.canvas.collapse_all()
            self.application.processEvents()
            self.assertIn("cmd", window.canvas.collapsed_folders)
            QTest.keyClick(window.canvas.focused_node, Qt.Key.Key_Right, Qt.KeyboardModifier.MetaModifier)
            self.application.processEvents()
            self.assertNotIn("cmd", window.canvas.collapsed_folders)
            QTest.keyClick(window.canvas.focused_node, Qt.Key.Key_Left, Qt.KeyboardModifier.MetaModifier)
            self.application.processEvents()
            self.assertIn("cmd", window.canvas.collapsed_folders)
            window.canvas.expand_all()
            self.application.processEvents()
            self.assertFalse(window.canvas.collapsed_folders)
            window.canvas.zoom_out()
            self.assertFalse(window.canvas.auto_fit)
            self.assertLess(window.canvas.zoom, 1.0)
            window.canvas.fit_canvas()
            self.assertTrue(window.canvas.auto_fit)
            self.assertEqual(window.canvas.zoom, 1.0)
